...
<div class="page-content">
	<form action="<%=path%>/book/queryNameForUser" class="form-inline" method="post">
			...
			<div class="panel-body">
				<div class="pull-left">
					<div class="form-group qinfo">
						...
						<input name="name" value="${queryParam.name}" class="form-control">
					</div>
			...
			<table class="table table-striped table-hover"
				...
				<c:forEach items="${pageInfo.list}" var="book">
					<tr>
						...
						<td>${book.grade}</td>
						<td>
							<a href="<%=path%>/book/toView?bookId=${book.id}" class="btn btn-success btn-xs">
								<span class="glyphicon glyphicon-eye-open"></span>查看
							</a>
							<button type="button" id="addToMyBookshelf" class="btn btn-warning btn-xs"
									onclick="addToMyBookshelf('${book.id}');">
			...
			<nav aria-label="Page navigation">
				<ul class="pagination">
					<c:if test="${pageInfo.hasPreviousPage}">
						<li>
							<a href="${pageContext.request.contextPath}/book/queryNameForUser?page=${pageInfo.prePage}&rows=${pageInfo.pageSize}
					...
					<c:forEach items="${pageInfo.navigatepageNums}" var="nav">
						...
						<c:if test="${nav != pageInfo.pageNum}">
							<li>
								<a href="${pageContext.request.contextPath}/book/queryNameForUser?page=${nav}&rows=${pageInfo.pageSize}
					...
					<c:if test="${pageInfo.hasNextPage}">
						<li>
							<a href="${pageContext.request.contextPath}/book/queryNameForUser?page=${pageInfo.nextPage}&rows=${pageInfo.pageSize}
...
<script type="text/javascript">
	$(document).ready(function(){
		$("#checkWord").click(function(){
			window.location.href = '<%=path%>/word/userWords';
		});
	});

	function addToMyBookshelf(id){
		$.ajax({
			url:'<%=path%>/bookshelf/add',
			type:'GET',//GET 或POST
