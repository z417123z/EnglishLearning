					...
					<tr>
						...
						<td>
							...
							<c:if test="${comment.showCon==0}">
								<a href="<%=path%>/comment/showContent?id=${comment.id}" class="btn btn-warning">
				...
				<nav aria-label="Page navigation">
					<ul class="pagination">
						<c:if test="${pageInfo.hasPreviousPage}">
							<li>
								<a href="${pageContext.request.contextPath}/comment/toQueryContentList?page=${pageInfo.prePage}&rows=${pageInfo.pageSize}
									&content=${content}" aria-label="Previous">
						...
						<c:forEach items="${pageInfo.navigatepageNums}" var="nav">
							...
							<c:if test="${nav != pageInfo.pageNum}">
								<li>
									<a href="${pageContext.request.contextPath}/comment/toQueryContentList?page=${nav}&rows=${pageInfo.pageSize}
						...
						<c:if test="${pageInfo.hasNextPage}">
							<li>
								<a href="${pageContext.request.contextPath}/comment/toQueryContentList?page=${pageInfo.nextPage}&rows=${pageInfo.pageSize}
									&content=${content}" aria-label="Next">
